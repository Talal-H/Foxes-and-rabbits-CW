 ...
 * Megalodon age, move, eat rabbits, and die.
 * 
...
public class Megalodon extends Animal
{
    ...
    private static final double NIGHT_MOVING_PROB = 0.2;
    // The maximum number of births.
    private static final int MAX_LITTER_SIZE = 2;
    // The food value of a single rabbit. In effect, this is the
    // number of steps a megalodon can go before it has to eat again.
    ...
    // The megalodon's age.
    private int age;
    // The megalodon's food level, which is increased by eating rabbits.
     ...
     */
    public void act(List<Animal> newMegalodon)
    {
        incrementAge();
        incrementHunger();
        if(isAlive() && getField().getIsDay()) {
            findMate(newMegalodon);
    ...
    {
        if (foodLevel < RABBIT_FOOD_VALUE/2){
            hunt();
        }
    ...
    {
        if (food+foodLevel > HUNGER_CAP){
            foodLevel = HUNGER_CAP;
        }
        else{
            foodLevel += food;
        }
