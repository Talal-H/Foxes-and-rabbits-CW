...
public class Field
{
    private static final int MAX_KRILL = 10;
    ...
    public void populateKrill(/*int depth, int width*/)
    {
        for(int row = 0; row < depth; row++) {
            for(int col = 0; col < width; col++) {
                krill[row][col] = rand.nextInt(MAX_KRILL+1);
            }
        }
    }
    ...
    public void growKrill()
    {
        for(int row = 0; row < depth; row++) {
            for(int col = 0; col < width; col++) {
                if (rand.nextDouble() <= GROWTH_PROBABILTY && krill[row][col] < MAX_KRILL){
    ...
    public int eatKrill(int appetite)
    {
        for(int row = 0; row < depth; row++) {
            for(int col = 0; col < width; col++) {
                int krillCount = krill[row][col];
                if (krillCount > 0) {
                    if (appetite > krill[row][col]){
                        krill[row][col] = 0;
                        return krillCount;
                    }
                    else {
                        krill[row][col] -= appetite;
                        return appetite;
                    }
                }                
            }
